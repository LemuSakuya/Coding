...
def create_advanced_km_plot_quartiles(excel_filepath, threshold=0.04):
    # 设置和加载数据
    ...
    print(analysis_df[['T', 'E', 'BMI分组']].info())


    # 执行Log-rank检验
    results = logrank_test(
        analysis_df['T'].values,
        analysis_df['BMI分组'].values,
        analysis_df['E'].values
    )
    p_value = results.p_value
    print(f"  - Log-rank检验的P值为: {p_value:.4f}")
    ...
    ax1.text(0.1, 0.2, f'Log-rank P值 = {p_value:.4f}', transform=ax1.transAxes, fontsize=12, bbox=dict(boxstyle='round,pad=0.5', fc='wheat', alpha=0.5))
    
    ax1.set_title('Kaplan-Meier生存曲线及风险集表格 (按四分位数BMI分组)', fontsize=16)
    ax1.set_ylabel(f'生存概率 (Y染色体浓度 >= {threshold*100}%)', fontsize=12)
    ax1.legend(title="BMI 分组")
    ax1.grid(True, linestyle='--', alpha=0.6)

    add_at_risk_counts(*kmf_models, ax=ax2)
    
    ax2.set_xlabel('检测孕周 (周)', fontsize=12)
...
if __name__ == '__main__':
    ...
    create_advanced_km_plot_quartiles(excel_filepath)
